...
> **As of 2026-05-14:** open upstream PRs — WhatsApp owner-forward stack ([#16445](https://github.com/NousResearch/hermes-agent/pull/16445) → … → busy-path PR) via `fix/busy-path-pre-gateway-dispatch-upstream`, [#17165](https://github.com/NousResearch/hermes-agent/pull/17165) (WhatsApp inbound text batching) via `feature/whatsapp-inbound-text-batch`, [#18413](https://github.com/NousResearch/hermes-agent/pull/18413) (OpenCode auxiliary `/v1`) via `fix/auxiliary-opencode-strip-trailing-v1`, [#20713](https://github.com/NousResearch/hermes-agent/pull/20713) (end-turn tool contract) via `feature/end-turn-tool-contract-upstream`, [#20747](https://github.com/NousResearch/hermes-agent/pull/20747) (launcher symlink clobber) via `fix/launcher-symlink-clobber-upstream`, diagnostic-status leak + end-turn nudge correction via `fix/diagnostic-status-leak-and-end-turn-nudge`, and [#25119](https://github.com/NousResearch/hermes-agent/pull/25119) (queued follow-up media delivery) via `fix/queued-followup-media-delivery-upstream`. [#25183](https://github.com/NousResearch/hermes-agent/pull/25183) (queued follow-up transcript history) landed upstream (`9a815b6c8`, `cc64a04f6`); fork branch `fix/queued-followup-history-persistence-upstream` is no longer in `FEATURE_BRANCHES`. Drop each remaining branch from `FEATURE_BRANCHES` once merged. Otherwise `main = upstream/main + ops-overlay + cherry-picked feature branches`.
...
| [NousResearch/hermes-agent#20747](https://github.com/NousResearch/hermes-agent/pull/20747) | `fix/launcher-symlink-clobber-upstream` | **Open.** Prevent install-time launcher writes from clobbering `venv/bin/hermes` when `~/.local/bin/hermes` is symlinked. |
| *(pending upstream PR)* | `fix/diagnostic-status-leak-and-end-turn-nudge` | **Open.** Diagnostic status leak fix plus end-turn nudge correction, carried in the fork while upstream PR prep lands. |
